...
public class Amounts {
    ...
    public void depositCash(){
        long amount = 0;
        for (int i = 0; i < cell.length; i++) {
            String txt = "Enter count " + cell[i].getNominal() +" (max "+ (cell[i].getCount_Max() - cell[i].getCount()) +"):";
            int count = new EnterATM().startATMInt(txt);
            while (count > cell[i].getCount_Max() - cell[i].getCount()  ){
                 count = new EnterATM().startATMInt(txt);
            }
            cell[i].setCount(count);
            amount = amount + cell[i].getCount()*cell[i].getNominal();
        }
